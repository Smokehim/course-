...
app.get('/getusers',(req,res)=>{
    ...
    db.query(sql, (error,result)=>{
        if(error) return res.status(404).json({message:"not able to get users", error}) 
...
app.delete('/deleteusers/:id',(req,res)=>{
    ...
    db.query(sql,[id], (error,result)=>{
        if(error) return res.status(404).json({message:"not able to delete users", error})
         console.log("Requested userID:", id);
        console.log("Query result:", result);
...
app.post('/postquestion',(req,res)=>{
    ...
    db.query(sql,[text], (error,result)=>{
        if(error) return res.status(404).json({message:"not able to insert question", error}) 
        res.status(200).json({message:"question have been insert", result})
    })
})
app.put('/putquestion/:id',(req,res)=>{
    const {text, questionID} = req.body
    if(!questionID) return res.status(404).json({message:"not able to get questionID"})
    const sql =`UPDATE Question SET text=  COALESCE (?, text) WHERE questionID=?`
    db.query(sql,[text, questionID], (error,result)=>{
        if(error) return res.status(404).json({message:"not able to update question", error}) 
...
app.get('/getquestion',(req,res)=>{
    ...
    db.query(sql, (error,result)=>{
        if(error) return res.status(404).json({message:"not able to get question", error}) 
        res.status(200).json({message:"question have been get", result})
    })
})
app.get('/getquestion/:id',(req,res)=>{
    const id = req.body.questionID
    if(!id) return res.status(404).json({message:"not able to get questionID"})
...
app.delete('/deletequestion/:id',(req,res)=>{
    ...
    db.query(sql,[id], (error,result)=>{
        if(error) return res.status(404).json({message:"not able to DELETE question", error})
         console.log("Requested question:", id);
        console.log("Query result:", result);
...
app.post('/postOption',(req,res)=>{
    const {text, questionID} = req.body
    if(!questionID || !text) return res.status(404).json({message:"not able to get questionID"})
    const sql =`INSERT INTO Option(text, questionID) VALUES (?, ?)`
    db.query(sql, [text, questionID], (error, result)=>{
        if(error) return res.status(404).json({message:"not able to insert Option", error})
        if(!result.affectedRows) return res.status(400).json({message:"no rows affected"}); 
...
app.put('/putOption/:id',(req,res)=>{
    ...
    if(!optionID) return res.status(404).json({message:"not able to get OptionID"})
    const sql =`UPDATE Option SET text=  COALESCE (?, text) WHERE optionID=?`
    db.query(sql,[text, optionID], (error,result)=>{
        if(error) return res.status(404).json({message:"not able to update Option", error})
        if(!result.affectedRows) return res.status(400).json({message:"no rows affected"}); 
...
app.get('/getOption',(req,res)=>{
    ...
    db.query(sql, (error,result)=>{
        if(error) return res.status(404).json({message:"not able to get Option", error}) 
...
app.post('/postSuggestion', async(req,res)=>{
    ...
    if(!userID || !text) return res.status(404).json({message:"not able to get questionID and text"})
    ...
    db.query(sql, values, (error, result)=>{
        if(error) return res.status(404).json({message:"not able to insert Suggestion", error})
        if(!result.affectedRows) return res.status(400).json({message:"no rows affected"}); 
...
app.put('/putSuggestion/:id',(req,res)=>{
    ...
    if(!suggestionID) return res.status(404).json({message:"not able to get SuggestionID"})
    const sql =`UPDATE Suggestion SET text=  COALESCE (?, text) WHERE suggestionID=?`
    db.query(sql,[text, suggestionID], (error,result)=>{
        if(error) return res.status(404).json({message:"not able to update Suggestion", error})
        if(!result.affectedRows) return res.status(400).json({message:"no rows affected"}); 
        res.status(200).json({message:"Option have been updated", result})
    })
})
app.get('/getSuggestion',(req,res)=>{
    ...
    db.query(sql, (error,result)=>{
        if(error) return res.status(404).json({message:"not able to get Suggestion", error}) 
...
app.get('/getSuggestion/:id',(req,res)=>{
    ...
    if(!id) return res.status(404).json({message:"not able to get suggestionID"})
    const sql =`SELECT * FROM Suggestion WHERE suggestionID=?`
    db.query(sql,[id], (error,result)=>{
        if(error) return res.status(404).json({message:"not able to getSuggestion", error})
...
app.delete('/deleteSuggestion/:id',(req,res)=>{
    ...
    db.query(sql,[id], (error,result)=>{
        ...
        res.status(200).json({message:"here is your deleted Suggestion", result})
    })
})
//answers
...
app.put('/putAnswers/:id',(req,res)=>{
    ...
    if(!answersID) return res.status(404).json({message:"not able to get SuggestionID"})
    ...
    db.query(sql,[text, answersID], (error,result)=>{
        if(error) return res.status(404).json({message:"not able to update Answers", error}) 
        if(!result.affectedRows) return res.status(400).json({message:"no rows affected"});
...
app.get('/getAnswers',(req,res)=>{
    ...
    db.query(sql, (error,result)=>{
        if(error) return res.status(404).json({message:"not able to get Answers", error}) 
        res.status(200).json({message:"Answers have been get", result})
    })
})
app.get('/getSuggestion/:id',(req,res)=>{
    const id = req.body.answersID
    if(!id) return res.status(404).json({message:"not able to get AnswersID"})
...
app.delete('/deleteAnswers/:id',(req,res)=>{
    ...
    db.query(sql,[id], (error,result)=>{
        if(error) return res.status(404).json({message:"not able to delete Answers", error})
        console.log("RequestedSuggestion:", id);
        console.log("Query result:", result);
